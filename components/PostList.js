...
function PostList({ posts, handleDelete, handleEdit }) {
  return (
    <div>
      {posts.map((post, index) => (
        <div
          key={`${post.tite}+${index}`}
          className='flex flex-row w-full h-52 bg-white py-3'
        >
          ...
          <div className='articleRight'>
            ...
            <p className='font-normal text-xs mb-5'>{post.description}</p>
          </div>
        </div>
      ))}
    </div>
